...
void checkDuplicate(entry_qb *list, entry_qb *head);
...
void checkDuplicate(entry_qb *list, entry_qb *head){
	for(entry_qb *temp = head; temp != head -> prev; temp = temp -> next){
		//Search and delete combo after you initialize list.
		//starting at temp and staying there. check if temp checks firstname == temp first name and increment the temp next if its not true
		for(entry_qb *tempCheck = temp; tempCheck -> firstName == temp -> firstName; temp = temp -> next){
			if ((tempCheck->firstName == temp -> firstName) && (tempCheck -> lastName == temp -> lastName)){
				temp -> wins++;//this is a new temp every time, so it increments to 1 always and the if statements do not execute.
				if(temp -> wins == 2){
					temp -> arrayYear[1] = temp -> yearWon;
				}
				if(temp -> wins == 3){
					temp -> arrayYear[2] = temp -> yearWon;
				}
				if(temp -> wins == 4){
					temp -> arrayYear[3] = temp -> yearWon;
				}
			}
