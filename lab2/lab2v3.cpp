...
char printOut;;//Prints out the final string from the schedule funciton.
/*===============*/
...
while(1){
	...
	if(menuSel==2){
		...
		/*=====  This assigns the day of the week to an integer  ====*/
		time_t now = time(0);
		tm * ltm = localtime(&now);
		day = ltm->tm_wday;

		/*=== This is taking the integer from the day of the week and assigining the userInputDay char to the appropriate day ===*/
		if(day==0){userInputDay[10]= sunday[10];}
		if(day==1){userInputDay[10]= monday[10];}
		if(day==2){userInputDay[10]= tuesday[10];}
		if(day==3){userInputDay[10]= wednesday[10];}
		if(day==4){userInputDay[10]= thursday[10];}
		if(day==5){userInputDay[10]= friday[10];}
		if(day==6){userInputDay[10]= saturday[10];}
